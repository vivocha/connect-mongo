...
module.exports = function(connect) {
  ...
  function MongoStore(options) {
    ...
    function connectionReady(err) {
      ...
      self.safe = self.db.collection(
        options.collection,
        {
          slaveOk: false,
          readPreference: mongo.Server.READ_PRIMARY
        }
      );
    ...
    this.getCollection = function (done, safe) {
      switch (self.state) {
        case 'connected':
          done(null, safe ? self.safe : self.collection);
          break;
        case 'connecting':
  ...
  MongoStore.prototype.get = function(sid, callback) {
    ...
    function getData(err, collection) {
      if (err) return callback(err);
      //console.debug('MongoStore get', collection.readPreference, 'try', _safe);
      collection.findOne(query, function(err, session) {
        ...
        if (session) {
          ...
          _safe++;
          self.getCollection(getData, true);
        } else {
          callback();
        }
      });
